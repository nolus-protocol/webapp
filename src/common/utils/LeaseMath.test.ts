...
import { LeaseMath } from "./LeaseMath";
...
describe("LeaseMath.calculateLiquidation", () => {
  it("should compute unit / price / 0.9", () => {
    // 1000 / 10 / 0.9 = 111.111...
    const result = LeaseMath.calculateLiquidation(new Dec(1000), new Dec(10));
  ...
  it("should handle large unit values", () => {
    ...
    const result = LeaseMath.calculateLiquidation(new Dec("1000000000000000"), new Dec(1));
    expect(result.gt(new Dec("1000000000000000"))).toBe(true);
  });

  it("throws when price is zero (Dec.quo on zero denominator)", () => {
    expect(() => LeaseMath.calculateLiquidation(new Dec(1), new Dec(0))).toThrow(/Division by zero/);
  });
});

describe("LeaseMath.calculateLiquidationShort", () => {
  it("should compute unit * 0.9 / price", () => {
    // 1000 * 0.9 / 10 = 90
    const result = LeaseMath.calculateLiquidationShort(new Dec(1000), new Dec(10));
    expect(decEquals(result, new Dec(90))).toBe(true);
  });

  it("should return value equal to 0.9 of unit when price == 1", () => {
    const result = LeaseMath.calculateLiquidationShort(new Dec(500), new Dec(1));
    // 500 * 0.9 = 450
    expect(decEquals(result, new Dec(450))).toBe(true);
  });

  it("throws when price is zero (Dec.quo on zero denominator)", () => {
    expect(() => LeaseMath.calculateLiquidationShort(new Dec(1), new Dec(0))).toThrow(/Division by zero/);
  });
});

describe("LeaseMath.calculateAditionalDebt", () => {
  it("should compute principal * percent * 180 / 31_536_000", () => {
    // 1000 * 0.1 * 180 / 31536000 = 18000 / 31536000 ≈ 0.00057077625...
    const result = LeaseMath.calculateAditionalDebt(new Dec(1000), new Dec("0.1"));
  ...
  it("should return zero when percent is zero", () => {
    const result = LeaseMath.calculateAditionalDebt(new Dec(1000), new Dec(0));
    expect(result.isZero()).toBe(true);
  });

  it("should return zero when principal is zero", () => {
    const result = LeaseMath.calculateAditionalDebt(new Dec(0), new Dec("0.1"));
    expect(result.isZero()).toBe(true);
  });
});

describe("LeaseMath.additionalInterest", () => {
  it("should return Dec(0) when data is null", () => {
    const result = LeaseMath.additionalInterest(null);
    expect(result.isZero()).toBe(true);
  });
  ...
  it("should compute loan_rate/1000 + margin_rate/100 and feed into calculateAditionalDebt", () => {
    ...
    const result = LeaseMath.additionalInterest(data as unknown as OpenedLeaseInfo);
    const expected = LeaseMath.calculateAditionalDebt(new Dec(1000), new Dec("0.2"));
