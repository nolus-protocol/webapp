 ...
 * Phantom and Solflare both own SOLANA via the native Solana provider.
 * Ledger / Ledger BLE are sunset and intentionally unwired — callers must
 * NOT call `applyWalletProtocolFilter` on a Ledger connect path; this
 * function still returns `undefined` for them so the wrapper degrades to
 * "" cleanly.
 *
...
export function protocolFilterForMechanism(mechanism: WalletConnectMechanism | null | undefined): string | undefined {
  switch (mechanism) {
    case WalletConnectMechanism.KEPLR:
      return "OSMOSIS";
    case WalletConnectMechanism.SOL_PHANTOM:
