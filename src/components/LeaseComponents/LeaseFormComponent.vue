    ...
    <div class="modal-send-receive-input-area">
      ...
      <div class="flex mt-6 justify-between text-primary text-[14px] garet-medium">
        ...
        <p class="flex">
          ...
          <TooltipComponent
            :content="$t('message.lease-swap-fee-tooltip', { swap_fee: swapFee * 100 })"
          />
        </p>
      </div>

      <RangeComponent class="py-4 my-2" :disabled="false" @on-drag="onDrag"> </RangeComponent>
...
onMounted(() => {
  ...
  if (
    liquiStakeTokens[coinList.value[selectedIndex.value].ticker as keyof typeof liquiStakeTokens]
  ) {
...
const coinList = computed(() => {
  return props.modelValue.currentBalance
    .filter((item) => {
      ...
      const downPaymentCurrency =
        wallet.currencies[props.modelValue.selectedDownPaymentCurrency.balance.denom];
...
const setAmount = (p: number) => {
  ...
  const amount = CurrencyUtils.convertMinimalDenomToDenom(
    props.modelValue.selectedDownPaymentCurrency.balance.amount,
...
const percentLique = computed(() => {
  ...
  const currecy = wallet.getCurrencyByTicker(asset.ticker);
  const price = new Dec(
    oracle.prices[currecy!.ibcData as string]?.amount ?? "0",
    asset.coinDecimals
  );
