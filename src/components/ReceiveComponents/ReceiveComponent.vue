  ...
  <div class="modal-send-receive-input-area">
    <div class="block text-left">
      ...
      <div class="block mt-[18px]">
        ...
        <div class="flex items-center justify-start mt-2">
          <button 
            ...
            @click="modelValue?.onCopyClick(wallet);onCopy()"
          >
      ...
      <div class="flex justify-between w-full text-light-blue text-[14px] mt-4">
        <p>
          {{ $t('message.estimate-time') }}:
        </p>
        <p>
          -{{ selectedNetwork.estimation }} {{ $t('message.sec') }}
        </p>
      </div>
...
const wallet = computed(() => {
  return WalletUtils.transformWallet(walletStore.wallet?.pubKey as Uint8Array, selectedNetwork.value.prefix);
});
