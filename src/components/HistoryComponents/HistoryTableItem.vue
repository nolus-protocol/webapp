<template>
  <div class="history-item">
    <div
      ...
      :key="index"
      class="md:grid md:grid-cols-12 gap-6 border-b border-standart py-4 md:flex items-center text-12"
    >
      <div
      ...
      >
        ...
        <img class="float-right w-3 his-img mt-[0.15rem]" src="@/assets/icons/urlicon.svg" />
      </div>

      <div class="block col-span-6 nls-14 nls-font-400 text-primary text-left text-14">
    ...
    case Messages['/cosmwasm.wasm.v1.MsgExecuteContract']: {
      try {
        ...
        if (data.close_position?.partial_close) {
          const currency = wallet.getCurrencyByTicker(
...
const messages = () => {
  return props.transaction.msgs.filter((item) => {
    switch (item.typeUrl) {
      ...
      case Messages['/cosmos.staking.v1beta1.MsgDelegate']: {
        if (
          item.data.delegatorAddress != (wallet.wallet?.address ?? WalletManager.getWalletAddress())
        ) {
          return false
        }
        return true
