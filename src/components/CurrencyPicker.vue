      ...
      <div class="mt-1 relative picker-container icon">
        <ListboxButton
          ...
          <span class="flex items-center">
            <img :src="selected.value?.icon ?? getAssetInfo(selected.value?.balance?.denom)?.coinIcon"
        ...
        <transition name="collapse"
                    appear>
          <ListboxOptions class="absolute z-10 mt-1 background shadow-lg max-h-56 w-[125px] rounded-md text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm top-[46px] scrollbar"
                          v-if="optionsValue!.length > 0">
            <ListboxOption v-for="option in optionsValue"
                           :key="option.balance.denom"
                           :value="option"
                           v-slot="{ active, selected }"
                           as="template">
              <li class="cursor-default select-none relative py-2 pl-3 pr-9 dropdown-elements my-1"
                  :class="{ selected: selected }">
                <div class="flex items-center">
                  <img :src="option.icon ?? getAssetInfo(option.balance.denom).coinIcon"
