<template>
  ...
  <Modal v-if="state.showModal" @close-modal="state.showModal = false">
    <component :is="modalOptions[state.modalAction]" :selectedAsset="state.selectedAsset"/>
  </Modal>
</template>
...
const mainAssets = computed(() => useStore().state.wallet.balances)
const manipulatedAssets = computed(() => state.value.showSmallBalances ? mainAssets.value : filterSmallBalances(mainAssets.value))
