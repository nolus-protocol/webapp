...
export function useSharePnLDialog() {
  const dialog = ref<{ show: () => void } | null>(null);
  ...
  const generateSmallCanvas = (canvasElement: HTMLCanvasElement, imgSrc: string) =>
    scheduleRender(canvasElement, imgSrc);
  ...
  const show = async (data: LeaseInfo, displayData?: LeaseDisplayData) => {
    leaseData = data;
